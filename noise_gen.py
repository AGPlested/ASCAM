...
def oneplusf_gen(
    spectrum_size=1e6, sample_rate=50000, type="1+f", corner=1000, Verbose=False
):
    """
    ...
    spectrum.real = real_part * 2 - 1.0
    spectrum.imag = imag_part * 2 - 1.0
...
def spec_to_noise(spectrum, noise_sd=None, Verbose=False):
    ...
    if noise_sd != None:
        ...
        if Verbose:
            print("norm_noise:", norm_noise)
        noise_array = norm_noise * len(norm_noise)  # because of numpy fft scaling

        if Verbose:
            print("Scaling noise array by {} fA ".format(noise_sd))
