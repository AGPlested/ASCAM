...
class FirstActivationFrame(tk.Frame):
    def __init__(self, parent):
        ...
        if self.parent.data.fa_threshold is None and np.any(parent.data._tc_thresholds):
            ...
            self.parent.data.fa_threshold = np.mean(self.parent.data.episode.trace)
        ...
        if not self.tracking_on:
            ...
            if self.manually_set_eps:
                ...
                tk.Label(
                    warning,
                    text=(
                        "You have manually located the first "
                ...
                tk.Button(
                    warning,
                    text="Detect for all episodes",
                    command=lambda *args: [
                        self.manually_set_eps.clear(),
                        warning.destroy(),
                    ],
                ).grid(row=1)
                tk.Button(
                    warning,
                    text="Detect for remaining episodes",
    ...
    def create_widgets(self):
        ...
        self.toggle_button = tk.Button(
            self, text="Set Threshold", command=self.toggle_tracking
        )
        self.toggle_button.grid()
        ttk.Entry(self, textvariable=self.fa_threshold, width=10).grid(row=0, column=1)
        ttk.Label(self, text=f"[{self.parent.data.tc_unit}]").grid(row=0, column=2)

        self.manual_button = tk.Button(
            self, text="Mark events manually", command=self.toggle_manual_mode
        )
        self.manual_button.grid(row=1)
        ttk.Label(self, text="Clicking moves to next episode: ").grid(row=1, column=1)
        ...
        ttk.Button(self, text="Cancel", command=self.click_cancel).grid(row=5, column=1)
    ...
    def toggle_manual_mode(self):
        ...
        if not self.manual_mode:
            ...
            self.plot_manual_cid = self.parent.plots.fig.canvas.mpl_connect(
                "button_press_event", self.manual_fa_selection
            )
