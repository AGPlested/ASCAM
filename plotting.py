    ...
    **kwargs,
    ...
    if select_piezo:
        for piezo, trace in zip(piezos, traces):
            _, _, trace_points = piezo_selection(time, piezo, trace, active, deviation)
            trace_list.extend(trace_points)
    elif intervals:
        for trace in traces:
            _, trace_points = interval_selection(
                time, trace, intervals, sampling_rate, time_unit
            )
            trace_list.extend(trace_points)
    else:
        trace_list = traces
...
def plot_histogram(
    ...
    **kwargs,
    ...
    heights_single, bins_single, center_single, width_single = hist_single
    if allpoint_hist:
        logging.info("""will create allpoint histogram""")
        # get a list of all the currents and all the traces
        if episode_inds.size == 0:
            episode_inds = list(range(len(series)))
        all_piezos = [
            episode.piezo for episode in series if episode.n_episode in episode_inds
        ]
        all_traces = [
            episode.trace for episode in series if episode.n_episode in episode_inds
        ]
    ...
    if single_hist:
        ...
        ax.barh(center_single, heights_single, width_single, align="center", alpha=1)
...
def plot_traces(
    ...
    if show_command and type(episode.command) is np.ndarray:
        ...
        if t_zero != 0:
            plt.axvline(0, c="r", lw=1, ls="--", alpha=0.8)
    ...
    if show_idealization and episode.idealization is not None:
        plotTrace(ax=current_plot, time=time, trace=episode.idealization, alpha=0.6)
    # label only the last axis
    if show_command:
        current_plot.set_xticklabels([])
    else:
        current_plot.set_xlabel(f"Time [{time_unit}]")
    if t_zero != 0:
        plt.axvline(0, c="r", lw=1, ls="--", alpha=0.8)
